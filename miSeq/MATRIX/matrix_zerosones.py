...
#metadata
    ...
    if l.startswith('#CHROM'): #'#CHROM' is the header
        ...
        SAMPLES_HEADER = []
        for sample in SAMPLES: SAMPLES_HEADER = SAMPLES_HEADER + [sample]
        AF_samplenames = []
...
# Extract information per sample
    ...
    for SAMPLE in SAMPLES:
        ...
        if SAMPLE_INFO['GT'] != './.':
            SAMPLE_INFO_fields = SAMPLE_INFO_fields + [1] # SAMPLE_INFO['AD'][0], SAMPLE_INFO['AD'][1] are ref and alt allelic depths, respectively
        else:
            SAMPLE_INFO_fields = SAMPLE_INFO_fields + [0]
...
# Create a column to sum all samples containing a certain variant. So, for each row of the dataframe, sum all 0's and 1's = df.loc[:,SAMPLES_HEADER].sum()
df['sum_samples'] = df.loc[:,SAMPLES_HEADER].sum(axis=1)
...
filt = df
#filt = df.loc[(df['FILTER'] == 'PASS') | (df['FILTER'] == 'clustered_events')] # (is 'PASS' OR 'clustered_events')
...
#for sampleAF in AF_samplenames: filt = filt.loc[(filt[sampleAF]  >= 0.15) | (np.isnan(filt[sampleAF])) ]
...
# Find transcripts in our results that are stored in APPRIS_data.principal database
# Ángel's way
