...
#################### CREATE 0's & 1's MATRIX #####################

AF_samplenames_gt= []
OUTPUT_HEADER = ['ID','CHROM','POS','REF','ALT','FILTER','DP_allsamples','Allele','Consequence','IMPACT','SYMBOL','Gene','Feature_type','Feature','BIOTYPE','EXON','cDNA_position','VARIANT_CLASS','HGNC_ID','Protein_position','Amino_acids','Codons','Existing_variation','gnomAD_AF','gnomAD_AFR_AF','gnomAD_AMR_AF','gnomAD_ASJ_AF','gnomAD_EAS_AF','gnomAD_FIN_AF','gnomAD_NFE_AF','gnomAD_OTH_AF','gnomAD_SAS_AF','CLIN_SIG','SOMATIC','PHENO'] # check the INFO header of your VCF for more ouput options
AF_samplenames_gt = [x + '_gt' for x in SAMPLES] # Create a list with the sample names and '_gt'
OUTPUT_HEADER_01 = OUTPUT_HEADER + ['CCDS'] + ['APPRIS']

 #Filters
df_01 = filt[filt['Consequence'].str.contains('missense_variant|frameshift_variant|stop_gained|missense_variant&splice_region_variant|missense_variant&NMD_transcript_variant|stop_gained&NMD_transcript_variant|inframe_deletion|stop_gained&frameshift_variant|stop_gained&frameshift_variant&NMD_transcript_variant|missense_variant&splice_region_variant&NMD_transcript_variant|stop_lost|inframe_insertion|inframe_insertion&NMD_transcript_variant')==True]

# Split fd_01 into the general info and the Sample info columns
df_01_AF_samplenames_gt = df_01[AF_samplenames_gt].copy()
df_01_HEADER = df_01[OUTPUT_HEADER_01].copy()



t = list(pd.unique(df_01_AF_samplenames_gt[AF_samplenames_gt].values.ravel()))
for i in t:
    if i == './.':
        df_01_AF_samplenames_gt = df_01_AF_samplenames_gt[AF_samplenames_gt].replace(i,0)
    else:
        df_01_AF_samplenames_gt = df_01_AF_samplenames_gt[AF_samplenames_gt].replace(i,1)

df_01_AF_samplenames_gt = df_01_AF_samplenames_gt[AF_samplenames_gt].apply(pd.to_numeric)
df_01_AF_samplenames_gt = pd.concat([df_01_HEADER,df_01_AF_samplenames_gt],axis=1)
df_01['n_samples'] = df.loc[:,AF_samplenames_gt].sum(axis=1)
df_01_AF_samplenames_gt['n_samples'] = df_01_AF_samplenames_gt.loc[:,AF_samplenames_gt].sum(axis=1)
...
##################### SAVE RESULTS MATRIX 01's #####################



# outdir = '/home/jlanillos/Disco4tb//Projects/miSeq/src/MATRIX20180820/results/' # testing the code
outdir = '/home/jlanillos/Disco4tb/Projects/miSeq/results_SAMPLES3/CORRECTED/MATRIX_zeroesones/'

# zeroes and ones matrix
df_01_AF_samplenames_gt.to_csv(''.join([outdir,'MATRIX.csv']),sep='\t')
